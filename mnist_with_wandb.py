...
def make_MNIST_dataset(train, config, download=True):
    ...
    if train:
        shuffler = StratifiedShuffleSplit(n_splits=1, test_size=config.valid_split, random_state=42).split(mnist.data, mnist.targets)
        ...
        trainds = MNISTTensorDataset(X_train, y_train.long(), config.train_transform)
        validds = MNISTTensorDataset(X_valid, y_valid.long(), config.test_transform)
        
        return trainds, validds
    
    else:
        X_test, y_test = mnist.data, mnist.targets
        testds = MNISTTensorDataset(X_test, y_test.long(), config.test_transform)
        return testds
...
def set_up(config):
    ...
    trainds, validds = make_MNIST_dataset(train=True, config=config, download=True)
